...
void GUI::centerText(const char* src)
{
  ...
  const byte len_src = strlen_P(src);
  ...
  for (i=0;i<LCD_COLUMNS_;i++)
  {
    ...
    {
      char cToStr[2];
      cToStr[0] = pgm_read_byte_near(src + j);
      cToStr[1] = '\0';
...
int GUI::displayIntPrompt(String msg, String unit,int start, int min, int max, int step)
{
...
bool GUI::displayYNPrompt(String msg, bool dftYes)
{
  Button_t buttonPressed;      // Contient le bouton appuyé
  byte shouldExitMenu = false; // Devient true quand l'utilisateur veut quitter le menu
...
void GUI::displayText(String msg, String title, bool waitKey)
{ 
	char *running;
	char *token;
